...
function onAdd(){
	...
	let stu_num = document.querySelector('.stu_num').value;
	//2.2 <input>에 입력된 value 가져와서 변수에 저장
/*	let stu_numValue = stu_num . value*/
	//2. 확인
		console.log('2.입력값 확인: ' + stu_num) //2. 확인
	
	// ! 검사 변수 [유효성검사 체크리스트]
	let confirm = 0;
	
	//추가 코드 1. 입력 값 공백
	if(stu_num == ''){alert('학번을 입력 해주세요.'); confirm++}
	//추가 코드 2. 입략 값 8자리x
	if(confirm ==0 && stu_num.length!=8){
	...
	if( confirm == 0) {/// 만약에 confirm 0 일때만 아래 코드 실행
			if(studentArray.indexOf(stu_num)==-1){ // 중복 없다
				//console.log('학번이 등록 되었습니다.')
				studentArray.push(stu_num)
